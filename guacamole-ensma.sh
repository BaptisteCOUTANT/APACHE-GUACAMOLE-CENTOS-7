## ----------------------------------- SCRIPT INSTALLATION GUACAMOLE ENSMA ---------------------------------------##
##																												  ##
##				EEEEEEEEEE   NNN           NN      SSSSSSSSS      MMMM     MMMM       AAAA                        ##
##				EEEEEEEEEE   NNNN          NN    SSSSS   SSSSS    MMMMM   MMMMM      AA  AA                       ##
##				EE           NN NNN        NN   SSSSS             MM MM   MM MM     AA    AA                      ##
##				EE           NN  NNN       NN    SSSSS            MM  MM MM  MM    AAA    AAA                     ##
##				EEEEEEEEEE   NN   NNN      NN      SSSSS          MM   MMM   MM    AA      AA                     ##
##				EEEEEEEEEE   NN    NNN     NN        SSSSSS       MM    M    MM   AAAAAAAAAAAA                    ##
##				EE           NN     NNN    NN           SSSSS     MM         MM   AAA      AAA                    ##
##				EE           NN        NNN NN             SSSSS   MM         MM   AAA      AAA                    ##
##				EEEEEEEEEE   NN          NNNN    SSSSS   SSSSS    MM         MM   AAA      AAA                    ##
##				EEEEEEEEEE   NN           NNN      SSSSSSSSS      MM         MM   AAA      AAA                    ##
##                                                                                                                ##
## -------------------------------------------------------------------------------------------------------------- ##

#Si le script n'est pas exécuté en tant que sudo ou root alors on envoi un message d'erreur.
if ! [ $(id -u) = 0 ]; then echo " - Ce script doit être exécuté en tant que sudo ou root, réessayez ..."; exit 1; fi

#Si le script n'a pas SELinux actif ou en mode Enforcing alors on envoi un message d'erreur.
if ! [ $(getenforce) = "Enforcing" ]; then echo "- Ce script nécessite que SELinux soit actif et en mode Enforcing"; exit 1; fi

#Si le script n'est pas exécuter en 64 bits alors on envoi un message d'erreur.
if ! [ $(uname -m) = "x86_64" ]; then echo " - Ce script ne fonctionnera que sur les versions 64 bits de CentOS"; exit 1; fi

#Si l'extension firewalld n'est pas installé sur le système alors on envoi un message d'erreur.
if ! rpm -q --quiet "firewalld"; then echo " - Ce script nécessite que firewalld soit installé sur le système"; exit 1; fi

#Autoriser trap à fonctionner dans les fonctions
set -E


# ----------------------------------------------------------- INFORMATIONS ------------------------------------------------------------------ #

#Date de la dernière modification du script.
VERSION_SCRIPT="2020_11_02"

#Message de contact d'administrateur.
ADMINISTRATEUR_CONTACT="Adminitrateur local, prenom.nom@ensma.fr"
...
#JAVA KAYSTORE : taille de la clé par défaut
JKSTORE_KEY_SIZE_DEF="4096"

#LET'S ENCRYPT : taille de la clé par défaut
LE_KEY_SIZE_DEF="4096"

#SELF-SIGNED SSL : taille de la clé par défaut
SSL_KEY_SIZE_DEF="4096"
...
MYSQL_MOT_DE_PASSE="8e1n2s3m4a56"

#NOM BASE DE DONNEE PAR DEFAUT
DB_NAME_DEF="db_ensma_guacamole"

#NOM UTILISATEUR BASE DE DONNEE PAR DEFAUT
DB_USER_DEF="db_ensma_adm"

#MOT DE PASSE BASE DE DONNEE PAR DEFAUT
DB_PASSWD_DEF="8e1n2s3m4a56"

#MOT DE PASSSE JAVA KEYSTORE PAR DEFAUT
JKS_GUAC_PASSWD_DEF="8e1n2s3m4a56"

#MOT DE PASSSE JAVA KEYSTORE LDAPS PAR DEFAUT
JKS_CACERT_PASSWD_DEF="8e1n2s3m4a56"
...
#SERVEUR DNS OCSP PAR DEFAUT. (Uniquement si l'hôte n'a pas de serveurs de noms dans resolv.conf).
GUACD_USER="guacd" 

#CHEMIN GUACAMOLE PAR DEFAUT.
GUAC_URIPATH_DEF="/"

#NOM DE DOMAIN PAR DEFAUT DU SERVEUR.
DOMAIN_NAME_DEF="localhost" #domaine par defaut 'localhost' mais il faut mettre 'ensma.fr'.

#DEFAUT DES ERREURS EN DESACTIVER (false).
H_ERR=false

#REMPLACEZ LA LIGNE EXCLUDE dans /etc/yum.repos.d/libjpeg-turbo.repo PAR CE QUI SUIT POUR EXCLURE LES VERSIONS BETA DE libjpeg - turbo
#utile si vous déployez libjpeg - turbo dans un environnement de production.
LIBJPEG_EXCLUDE="exclude=libjpeg-turbo-[0-9]*,libjpeg-turbo-*.*.9[0-9]-*"

#COMPORTEMENT PAR DEFAUT POUR SUPPRIMER LE FICHIER temp var UTILISE PAR LE GESTIONNAIRE D'ERREURS A LA FIN.
#(Définir sur false pour conserver le fichier afin de revoir les dernières valeurs)
DEL_TMP_VAR=true

#SERVEUR DNS OCSP PAR DEFAUT
#(Utilisé uniquement si l'hôte n'a pas de serveurs de noms dans resolv.conf)
NAME_SERVERS_DEF="1.1.1.1 1.0.0.1 2606:4700:4700::1111 2606:4700:4700::1001"

# ---------------------------------------------------------------------------------------------------------------------------------------------#



# ------------------------------------------------------- URL FICHIER OBLIGATOIRE -------------------------------------------------------------#

#LIEN DIRECT : MySQL et de la biblithéque JPEG sur site officiel.
...
#DOSSIER PROPRIETE GUACAMOLE
GUAC_CONF="guacamole.properties"

#RECHERCHE LA VERSION UTILISE DU CONNECTEUR JAVA
MYSQL_CON="mysql-connector-java-${MYSQL_CON_VER}"

#CHEMIN DES ERREURS GUACAMOLE
...
Magenta=`tput setaf 5`	#ROSE
Cyan=`tput setaf 6`		#BLEU CLAIR
White=`tput setaf 7`	#BLANC
Bold=`tput bold`		#GRAS (police)
UndrLn=`tput sgr 0 1`	#LIGNE 
Rev=`tput smso`			
Reset=`tput sgr0`		#REINITIALISATION
...
#VERSION GUACAMOLE GITHUB OFFICIEL APACHE GUACAMOLE	
...
#REPERTOIRE DANS LEQUEL ON SE TROUVE
PWD=`pwd`

#DEFINIT LE CHEMIN D'ACCES, ENREGISTREMENT DANS UN FICHIER DES VARIABLES TEMPORAIRES.
VAR_FILE="${PWD}/${TMP_VAR_FILE}"

#CREER UN FICHIER AVEC -1 POUR NE PAS DEFINIR COMME PROCESSUS D'ARRIERE-PLAN
echo "-1" > "${VAR_FILE}" 

#DETERMINE SUR QUELLE OS L'INSTALLER
...
#ERREUR, OS non pris en charge
else
	echo "Verification impossible, Script utilisable uniquement avec les distributions RHEL et CENTOS"
	exit 1
...
#RETOURNE LES VERSIONS MAJOR ET MINOR DE RHEL
...
#NGINX LIEN OFFICEL POUR CENTOS 7
...
if [ $GUAC_SOURCE == "Git" ]; then
	...
	#VERSION STABLE DE GUACAMOLE
	GUAC_VER=${GUAC_STBL_VER}

	#URL D'INSTALLATION OFFICIEL
...
#GENERATION FICHIER LOG NOM ENTIER.
logfile="${FILENAME}.log"

#FICHIER DE SAUVEGARDE DU FIREWALL
...
#SUPPRIME CE QUI EST AFFICHER A L'ECRAN
clear

#AFFICHAGE MENU
echo -e "   ${Reset}${Bold}ENSMA : Script d'installation APACHE GUACAMOLE 1.2.0\n       ${Reset}Passerelle de bureau à distance Guacamole\n"
echo -e "   ${Bold}***        MENU     ***\n"
echo "   OS: ${Yellow}${OS_NAME} ${MAJOR_VER}.${MINOR_VER} ${MACHINE_ARCH}${Reset}"
echo -e "   ${Bold}Version Stable : ${Yellow}${GUAC_STBL_VER}${Reset}\n"

#QUAND C'EST VRAI
while true; do
	echo -n "${Green} Veuillez taper 'stable' pour confirmer la version à installer : ${Yellow}"
	read GUAC_SOURCE
	case $GUAC_SOURCE in
		[Ss]table|"" ) GUAC_SOURCE="Stable"; break;;
		* ) echo "${Green} Veuillez entrer 'stable' pour sélectionner la source / version";;
	esac
done
...
# ------------------------------------------------ BASE DE DONNEES ET JAVA KEYSTORE ------------------------------------------------------------#

#MENU BASE DE DONNES 
db_menu () {
SUB_MENU_TITLE="MENU : BASE DE DONNES ET JAVA KEYSTORE"

#MENU
menu_header

#AFFICHAGE
echo -n "${Green} Entrez le nom de la base de données Guacamole (defaut ${DB_NAME_DEF}): ${Yellow}"
	read DB_NAME
	DB_NAME=${DB_NAME:-${DB_NAME_DEF}}
echo -n "${Green} Entrez le nom d'utilisateur de la base de données Guacamole (defaut ${DB_USER_DEF}): ${Yellow}"
	read DB_USER
	DB_USER=${DB_USER:-${DB_USER_DEF}}
echo -n "${Green} Entrez la taille de la clé Java KeyStore à utiliser (defaut ${JKSTORE_KEY_SIZE_DEF}): ${Yellow}"
...
#MENU MOT DE PASSE
pw_menu () {
SUB_MENU_TITLE="MENU : MOT DE PASSE"

#MENU
menu_header

#AFFICHAGE
echo -n "${Green} Entrez le mot de passe root pour MariaDB : ${Yellow}"
	read MYSQL_PASSWD
	MYSQL_PASSWD=${MYSQL_PASSWD:-${MYSQL_PASSWD_DEF}}
echo -n "${Green} Entrez le mot de passe Guacamole DB : ${Yellow}"
	read DB_PASSWD
	DB_PASSWD=${DB_PASSWD:-${DB_PASSWD_DEF}}
echo -n "${Green} Entrez le mot de passe Guacamole Java KeyStore, doit contenir au moins 6 caractères : ${Yellow}"
...
#MENU CERTIFICAT SSL
ssl_cert_type_menu () {
SUB_MENU_TITLE="MENU : CERTIFICAT SSL"

#MENU
menu_header

#AFFICHAGE
echo "${Green} Quel type de certificat SSL doit être utilisé (par défaut 2) ?${Yellow}"
PS3="${Green} Entrer le numéro du type de certificat désiré ${Yellow}"
...
do
	case $opt in
		...
		"None")
			SSL_CERT_TYPE="None"
			OCSP_USE=false
			echo -e "\n\n${Red} Aucun certificat SSL sélectionné. Cela peut être configuré manuellement ultérieurement."
			sleep 3
			break;;
		* ) echo "${Green} ${REPLY} n'est pas une option valide, entrez le nombre représentant le type de certificat souhaité.";;
		esac
done
}
...
#MENU LETSENCRYPT
le_menu () {
SUB_MENU_TITLE="MENU : LETSENCRYPT"

#MENU
menu_header

#AFFICHAGE
echo -n "${Green} Entrez un e-mail valide pour chiffrer le certificat : ${Yellow}"
	read EMAIL_NAME
echo -n "${Green} Entrez la taille de la clé Let's Encrypt à utiliser (defaut ${LE_KEY_SIZE_DEF}): ${Yellow}"
	read LE_KEY_SIZE
	LE_KEY_SIZE=${LE_KEY_SIZE:-${LE_KEY_SIZE_DEF}}

while true; do
	echo -n "${Green} Utiliser OSCP (par défaut : yes ): ${Yellow}"
	read yn
	case $yn in
		[Yy]*|"" ) OCSP_USE=true; break;;
		[Nn]* ) OCSP_USE=false; break;;
		* ) echo "${Green} Veuillez entrer yes ou no. ${Yellow}";;
		esac
done
}
...
ss_menu () {
OCSP_USE=false
SUB_MENU_TITLE="MENU : SELF-SIGNED SSL CERTIFICAT"

#ENTETE MENU
menu_header

echo -n "${Green} Entrez la taille de clé SSL auto-signée à utiliser (defaut ${SSL_KEY_SIZE_DEF}): ${Yellow}"
	read SSL_KEY_SIZE
...
echo -n "${Green} Entrez l'adresse IP LAN de ce serveur (defaut ${GUAC_LAN_IP_DEF}): ${Yellow}"
	read GUAC_LAN_IP
	GUAC_LAN_IP=${GUAC_LAN_IP:-${GUAC_LAN_IP_DEF}}
echo -n "${Green} Entrez un nom d'hôte ou un domaine public valide tel que ensma.fr (defaut ${DOMAIN_NAME_DEF}): ${Yellow}"
	read DOMAIN_NAME
	DOMAIN_NAME=${DOMAIN_NAME:-${DOMAIN_NAME_DEF}}
echo -n "${Green} Entrez le chemin de l'URI, commençant et se terminant par /parexemple/guacamole/ (defaut ${GUAC_URIPATH_DEF}): ${Yellow}"
	read GUAC_URIPATH
...
if [ $SSL_CERT_TYPE != "None" ]; then
	while true; do
		echo -n "${Green} Utilisez uniquement des chiffrements SSL> = 256 bits (plus sûrs, moins compatibles. Par défaut: yes)?: ${Yellow}"
		read yn
		case $yn in
			[Yy]*|"" ) NGINX_SEC=true; break;;
			[Nn]* ) NGINX_SEC=false; break;;
			* ) echo "${Green} Entrez s'il vous plait yes ou no. ${Yellow}";;
		esac
	done

	while true; do
		echo -n "${Green} Utilisez Content-Security-Policy [CSP] (Plus sécurisé, moins compatible. Par défaut: yes)?: ${Yellow}"
		read yn
		case $yn in
			[Yy]*|"" ) USE_CSP=true; break;;
			[Nn]* ) USE_CSP=false; break;;
			* ) echo "${Green} Veuillez entrer yes or no. ${Yellow}";;
		esac
	done
else
	NGINX_SEC=false
...
# ------------------------------------------------ AUTORISATION PRIMAIRE MENU ------------------------------------------------------------------#

#AUTORISATION PRIMAIRE MENU
prime_auth_ext_menu () {

SUB_MENU_TITLE="MENU : AUTORISATION PRIMAIRE"
...
echo "${Green} Quelle extension Guacamole doit être utilisée comme méthode d'authentification utilisateur principale (defaut 1)?${Yellow}"
PS3="${Green} Saisissez le numéro de la méthode d'authentification souhaitée: ${Yellow}"
...
do
	case $opt in
		...
		* ) echo "${Green} ${REPLY} n'est pas une option valide, entrez le nombre représentant la méthode d'authentification principale souhaitée.";;
		esac
done
